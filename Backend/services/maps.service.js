...
module.exports.getDistanceTime = async (
    ...
    const url =
        `https://api.geoapify.com/v1/routing?waypoints=${originCoords.ltd},${originCoords.lng}|${destCoords.ltd},${destCoords.lng}&mode=drive&apiKey=${GEOAPIFY_KEY}`;
    ...
    try {
        ...
        if (!feature) {
            ...
            throw new Error(
                'Unable to find a route between these locations'
            );
...
module.exports.getAutoCompleteSuggestions =
    async (input) => {
        ...
        const url =
            `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(input)}&apiKey=${GEOAPIFY_KEY}`;
